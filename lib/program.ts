...
export const PROGRAM: ProgramDay[] = [
  ...
  { day: 2,
    focusRu: 'Освой скольжение по тяге.',
    focusEn: 'Learn urge surfing.',
    scienceRu: 'Тягу можно пережить, просто наблюдая её как волну — это навык, а не сила воли (Marlatt).',
    scienceEn: 'A craving can be ridden out by watching it like a wave — a skill, not willpower (Marlatt).',
    taskRu: 'Переживи одну волну тяги, ничего не делая, — просто наблюдай, как она спадает.',
    taskEn: 'Ride out one craving wave doing nothing — just watch it pass.',
    practice: 'urge_surf',
  },
  { day: 3,
  ...
  { day: 12,
    focusRu: 'Перечитай свои триггеры.',
    focusEn: 'Revisit your triggers.',
    scienceRu: 'Триггеры смещаются. Сверяйся с дневником тяги раз в пару недель.',
    scienceEn: 'Triggers shift. Check your craving journal every couple of weeks.',
    taskRu: 'Открой дневник тяги — отметь, какие триггеры изменились.',
    taskEn: 'Open the craving journal — note which triggers have changed.',
    practice: 'ema',
  },
  { day: 13,
  ...
  { day: 23, focusRu: 'Алкоголь — главная ловушка.', focusEn: 'Alcohol is the main trap.',
    ...
    taskRu: 'Заранее реши: что закажешь в баре и куда денешь руки вместо сигареты.', taskEn: 'Decide in advance: what you order at the bar and what your hands do instead.', practice: 'urge_surf' },
  ...
  { day: 31, focusRu: 'Сверь свои триггеры.', focusEn: 'Check your triggers.',
    scienceRu: 'Триггеры меняются: появляются новые ситуации без сигареты.',
    scienceEn: 'Triggers shift: new smoke-free contexts appear.',
    taskRu: 'Открой дневник и отметь новые ситуации, где раньше курил.', taskEn: 'Open the journal and note new situations where you used to smoke.', practice: 'ema' },
...
export function methodFocus(stepId: StepLevel | undefined, day: number, lang: 'ru' | 'en'): { titleRu: string; titleEn: string; lineRu: string; lineEn: string } | null {
  ...
  if (stepId === 'L1_behavioral') {
    return {
      ...
      lineRu: 'Сегодня — техника. Дыхание утром, скольжение по тяге на главный триггер, чек-ин вечером.',
      lineEn: 'Today — technique. Morning breathing, urge surfing on your top trigger, evening check-in.',
    };
  }
