...
// Начало отсчёта для денег / «не выкурено» / вех здоровья — ВСЕГДА дата старта
// программы.
//
// 🔴 НЕ привязывать это к дню отказа по протоколу (preQuitGraceEnd). Так было
// сделано ради строгости: на фарме человек первые 4–7 дней ещё курит по схеме,
// значит «сэкономлено» вроде бы должно быть нулём. На практике это выглядело как
// сломанное приложение: юзер выбирает препарат — и главный экран показывает
// «0 ₽ / 0.0 сигарет» неделю, без единого слова почему. Ровно в тот момент, когда
// мотивация самая хрупкая. Плюс любая смена метода переписывает stepEnteredAt
// (app/method.tsx), и счётчики откатывались к нулю задним числом.
//
// Окно протокола по-прежнему учитывается там, где оно действительно важно и
// невидимо для счётчиков: детекция срыва (lib/relapse.ts, app/slip.tsx) и
// промпт ИИ (lib/ai.ts) через preQuitGraceEnd.
